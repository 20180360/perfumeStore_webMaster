...
const navigate = useNavigate();
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-4">
  {womenPerfumes.womenPerfumes.map((perfume) => (
    <div
      key={perfume.id}
      className="bg-white rounded-2xl shadow-lg group overflow-hidden flex flex-col items-center"
    >
      ...
      <div className="relative w-[300px] h-[300px]">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-30 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-4 transition-opacity duration-300">
          ...
          <button
            ...
            className="bg-white p-2 rounded-full shadow hover:bg-rose-100 transition"
          >
      ...
      <div className="p-4 flex flex-col justify-between w-full max-w-[300px]">
        <div>
          <h3 className="text-xl font-semibold text-gray-800">{perfume.name}</h3>
          <p className="text-sm text-gray-500">{perfume.brand}</p>
        </div>
        <div className="mt-2">
          <p className="text-lg font-bold text-amber-600">${perfume.price}</p>
          <button
            className="mt-2 w-full bg-amber-950 text-white py-1.5 rounded-lg hover:bg-rose-600 transition-colors"
